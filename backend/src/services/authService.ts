...
export async function registerUser(data: {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Criar token de verificação (expira em 24h)
    const expiresAt = new Date()
    expiresAt.setHours(expiresAt.getHours() + 24)

    const emailVerification = await tx.emailVerification.create({
      data: {
        userId: user.id,
        token: uuidv4(),
        expiresAt,
  ...
  // Enviar e-mail de verificação (não bloquear se falhar)
  try {
    await emailService.sendVerificationEmail({
      email: result.user.email,
      name: result.user.name,
      token: result.emailVerification.token,
    })
  } catch (error) {
